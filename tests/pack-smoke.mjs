...
const tempDir = await mkdtemp(path.join(os.tmpdir(), "browser-ops-pack-"));
let tarballPath;
...
try {
  ...
  const binPath = path.join(
    ...
    "node_modules",
    ".bin",
    process.platform === "win32" ? "browser-ops.cmd" : "browser-ops"
  );
  const { stdout } = await run(binPath, ["--help"], { cwd: tempDir });
  assert(stdout.includes("Usage: browser-ops [options] [command]"));
} finally {
